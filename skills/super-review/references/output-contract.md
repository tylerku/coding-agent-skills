...
3. `findings` when accepted findings remain and the lane is not blocked or owed.
4. `pass` when evidence is sufficient and no accepted findings remain.
5. `not_applicable` only when the dimension truly does not apply.

A higher-precedence status does not hide accepted findings; list them alongside the status. If all proposed findings are rejected and no evidence debt remains, report `pass`.
